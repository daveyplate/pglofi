    ...
    if (typeof options.on === "string") {
        ...
        if (detectedMany === undefined) {
            ...
            if (relatedHasColumn && !currentHasColumn) {
                ...
                if (
                    oneToMany &&
                    oneToMany.localColumn ===
                        tsToSqlColumn(relatedTable, options.on)
                ) {
                    detectedMany = true
                } else {
                    // Try many-to-one: FK on current table
                    const manyToOne = findForeignKey(
                        currentTable,
                        relatedTableName
                    )
                    if (
                        manyToOne &&
                        manyToOne.localColumn ===
                            tsToSqlColumn(currentTable, options.on)
                    ) {
                        detectedMany = false
                    } else {
        ...
        if (detectedMany === undefined) {
            ...
            if (localFieldOnCurrent && foreignFieldOnRelated) {
                ...
                const oneToMany = findForeignKey(
                    relatedTable,
                    currentTableName,
                    {
                ...
                if (oneToMany) {
                    ...
                    const manyToOne = findForeignKey(
                        ...
                        relatedTableName,
                        {
                            localColumn: tsToSqlColumn(
                                currentTable,
                                localField!
                            ),
                            foreignColumn: tsToSqlColumn(
                                relatedTable,
                                foreignField!
                            )
                        }
