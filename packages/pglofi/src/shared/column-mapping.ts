 ...
 *
 * @example
 * const data = { userId: '123', isComplete: true }
 * transformTsToSql(todosTable, data)
 * // Returns: { user_id: '123', is_complete: true }
 */
export function transformTsToSql(
    table: AnyPgTable,
...
/**
 * Transforms an array of rows from SQL to TypeScript property names.
 */
export function transformSqlRowsToTs<T = Record<string, unknown>>(
    table: AnyPgTable,
    sqlRows: Record<string, unknown>[]
): T[] {
    return sqlRows.map((row) => transformSqlToTs<T>(table, row))
}
 ...
 */
export function transformPostgrestRow<
    TSchema extends Record<string, AnyPgTable>
>(
    schema: TSchema,
    table: AnyPgTable,
