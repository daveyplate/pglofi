    ...
    const sync: SyncConfig<Row, Key> = {
        sync: (params: SyncParams) => {
            ...
            async function initialFetch() {
                 ...
                 */
                let cursor: RxDocumentData<Row> | undefined
                const syncBatchSize = config.syncBatchSize
        ...
        onInsert: async (params) => {
            ...
            return rxCollection.bulkUpsert(newItems).then((result) => {
                if (result.error.length > 0) {
                    throw rxStorageWriteErrorToRxError(
                        ensureNotFalsy(result.error[0])
                    )
                }
